...
class MS365MailReader:
    """Mail reading functionality class"""
    ...
    def search_emails(self, search_term, folder_name='Inbox', limit=10):
        ...
        folder = (self.mailbox.inbox_folder() if folder_name.lower() == 'inbox'
                 else self.mailbox.get_folder(folder_path=folder_name))
...
class EmailGUI(QMainWindow):
    ...
    def show_email_content(self, row, col):
        ...
        sender = self.emails_table.item(row, 1).text()
        date = self.emails_table.item(row, 2).text()
        
        content = f"Subject: {email}\nFrom: {sender}\nDate: {date}\n\n"
        self.email_content.setText(content)

    def show_search_content(self, row, col):
        email = self.search_results.item(row, 0).text()
        sender = self.search_results.item(row, 1).text()
        date = self.search_results.item(row, 2).text()
